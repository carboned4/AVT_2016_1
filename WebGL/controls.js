...
function handleMouseMove(event) {
 ...
 /*function handleMouseDown(event) {
        xFlare = event.clientX*gl.canvas.width/ window.innerWidth;
        yFlare = event.clientY*gl.canvas.height / window.innerHeight;
        if(event.which==3){
            mouseButton=1;
            mouseDown = true;
        }
        else if(cameraStyle==3 && event.which==1 ){ 
        mouseButton=0;
        mouseDown = true;
    }
        lastMouseX = event.clientX;
    ...
    /*function handleMouseMove(event) {
        ...
        mat4.translate(newRotationMatrix,[car.getPosX()-20*Math.cos(degToRad(car.getRot())), car.getPosY()-5, car.getPosZ()+2+20*Math.sin(degToRad(-car.getRot()))]);
        mat4.rotate(newRotationMatrix, degToRad(deltaX / 10), [0, 1, 0]);
             mat4.translate(newRotationMatrix,[-car.getPosX()+20*Math.cos(degToRad(car.getRot())), -car.getPosY()+5, -car.getPosZ()-2-20*Math.sin(degToRad(-car.getRot()))]);
        mat4.multiply(newRotationMatrix, moonRotationMatrix, moonRotationMatrix);


    /*var atX = car.getPosX()+(5)*Math.cos(degToRad(90));
    console.log(atX);

    var atZ = car.getPosZ()-(5)*Math.sin(degToRad(90));

    mat4.identity(mvMatrix);
    camera.rotate(atX,atZ);
    mat4.identity(pMatrix);
    camera.changeSize();*/
/*

        totalRotation += deltaX/10;
        lastMouseX = newX
        lastMouseY = newY;
        }

        else {
        xFlare += (newX - lastMouseX)*gl.canvas.width / window.innerWidth;
        yFlare += (newY - lastMouseY)*gl.canvas.height / window.innerHeight;
        lastMouseX = newX
        lastMouseY = newY;
